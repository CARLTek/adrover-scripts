...
#             detected_objects, _ = self.predictor.recognize(frame)
...
#         analytics['total_persons'] = detected_objects.n_persons
#         analytics['total_faces'] = detected_objects.n_faces

#         # Process person detections
#         person_inds = detected_objects.get_bboxes_inds("person")
#         for person_ind in person_inds:
#             person_data = self.extract_person_data_from_result(detected_objects, person_ind)
#             analytics['person_detections'].append(person_data)

#             # Update gender counts
#             if person_data['gender'] == 'male':
#                 analytics['male_count'] += 1
#             elif person_data['gender'] == 'female':
#                 analytics['female_count'] += 1
#             else:
#                 analytics['unknown_gender_count'] += 1

#             # Update age group counts
#             age_group = person_data['age_group']
#             if age_group == 'CHILD':
...
#             face_data = self.extract_face_data_from_result(detected_objects, face_ind)
#             analytics['face_detections'].append(face_data)

#         return analytics
    
#     def extract_person_data_from_result(self, detected_objects: PersonAndFaceResult, person_ind: int) -> dict:
#         """Extract data from a person detection using MiVOLO result structure"""
#         # Get bounding box from YOLO results
#         bbox = detected_objects.yolo_results.boxes[person_ind].xyxy[0].cpu().numpy()
#         confidence = detected_objects.yolo_results.boxes[person_ind].conf.cpu().numpy()[0]

#         # Get age and gender from MiVOLO results
...
#             bbox_norm = [bbox[0]/orig_w, bbox[1]/orig_h, (bbox[2]-bbox[0])/orig_w, (bbox[3]-bbox[1])/orig_h]
#         else:
#             bbox_norm = bbox.tolist()
...
#         return {
#             'bbox': [float(x) for x in bbox_norm],  # Convert to Python float
#             'age': float(age) if age is not None else None,  # Convert to Python float
#             'age_group': age_group,
#             'gender': gender if gender else 'unknown',
#             'confidence': float(confidence),
#             'gender_confidence': float(gender_score) if gender_score else None,
#             'track_id': None  # YOLO tracking not implemented in this version
#         }

#     def extract_face_data_from_result(self, detected_objects: PersonAndFaceResult, face_ind: int) -> dict:
#         """Extract data from a face detection using MiVOLO result structure"""
#         # Get bounding box from YOLO results
#         bbox = detected_objects.yolo_results.boxes[face_ind].xyxy[0].cpu().numpy()
#         confidence = detected_objects.yolo_results.boxes[face_ind].conf.cpu().numpy()[0]
...
#             # For unique person counting, we'll use a simple approach:
#             # Count unique persons based on consistent detection across frames
#             # If we consistently detect the same number of people, it's likely the same individuals

#             # Update frame-based statistics (these show activity per frame)
...
#             # This is a basic heuristic - in a real system you'd use proper tracking
...
#                 summary["estimated_unique_faces"] = most_common_count  # Assume 1 face per person
#             else:
...
#                 # For unique demographics, we'll take the most recent reliable reading
...
#             logger.info(f"Analytics data saved to {session_file}")
...
#             # Skip database operations for now to avoid async context issues
#             # TODO: Implement proper async database handling
#             logger.info(f"Analytics data: {analytics_data['total_persons']} persons, {analytics_data['total_faces']} faces, Ages: {[p.get('age') for p in analytics_data['person_detections']]}")
#             return

#             # Get or create camera session
#             session, created = CameraSession.objects.get_or_create(
#                 session_id=session_id,
#                 defaults={
...
#             # Create analytics frame record
#             frame_record = AnalyticsFrame.objects.create(
#                 session=session,
#                 frame_number=frame_number,
#                 total_persons=analytics_data['total_persons'],
#                 total_faces=analytics_data['total_faces'],
#                 male_count=analytics_data['male_count'],
#                 female_count=analytics_data['female_count'],
#                 unknown_gender_count=analytics_data['unknown_gender_count'],
#                 children_count=analytics_data['children_count'],
...
#             # Store individual person detections
#             for person_data in analytics_data['person_detections']:
#                 PersonDetection.objects.create(
#                     frame=frame_record,
#                     bbox_x=person_data['bbox'][0] if len(person_data['bbox']) > 0 else 0,
#                     bbox_y=person_data['bbox'][1] if len(person_data['bbox']) > 1 else 0,
#                     bbox_width=person_data['bbox'][2] if len(person_data['bbox']) > 2 else 0,
#                     bbox_height=person_data['bbox'][3] if len(person_data['bbox']) > 3 else 0,
#                     predicted_age=person_data['age'],
#                     age_group=person_data['age_group'],
#                     predicted_gender=person_data['gender'],
#                     person_confidence=person_data['confidence'],
#                     track_id=person_data['track_id']
#                 )
            
#             # Store individual face detections
#             for face_data in analytics_data['face_detections']:
#                 FaceDetection.objects.create(
#                     frame=frame_record,
#                     bbox_x=face_data['bbox'][0] if len(face_data['bbox']) > 0 else 0,
#                     bbox_y=face_data['bbox'][1] if len(face_data['bbox']) > 1 else 0,
#                     bbox_width=face_data['bbox'][2] if len(face_data['bbox']) > 2 else 0,
#                     bbox_height=face_data['bbox'][3] if len(face_data['bbox']) > 3 else 0,
#                     predicted_age=face_data['age'],
#                     predicted_gender=face_data['gender'],
#                     face_confidence=face_data['confidence']
#                 )
            
#             logger.info(f"Stored analytics for frame {frame_number} in database")
