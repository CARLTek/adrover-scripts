...
class UnifiedServer:
    def __init__(self, host: str = "0.0.0.0", port: int = 12350, engine: str = "mivolo"):
        ...
        self._init_db()
        # Show server-side visualization window with predictions
        self._show_window = True
        self._window_name = "Server Predictions"
        ...
        try:
            while True:
                ...
                if ad_id is not None:
                    payload["ad_id"] = ad_id
                # Optional server-side visualization
                if self._show_window:
                    try:
                        disp = frame.copy()
                        h, w = disp.shape[:2]
                        # Draw tracked persons (IDs and boxes) if available
                        for tp in analytics.get("tracked_persons", []) or []:
                            ...
                            if tid is not None:
                                cv2.putText(disp, f"ID {tid}", (x1, max(0, y1 - 6)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 200, 255), 1)
                        # Draw quick info panel
                        ...
                        def put(text):
                            nonlocal ytxt
                            cv2.putText(disp, text, (20, ytxt), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 255, 0), 1)
                            ytxt += 22
                        ...
                        cv2.resizeWindow(self._window_name, 960, 720)
